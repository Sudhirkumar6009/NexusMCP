...
const handlers: Record<string, MCPHandler> = {
  ...
  "jira.createIssue": async (params) => {
    ...
    const issueTitle = summary || title;
    if (!issueTitle) {
      throw new Error("Missing required field: summary or title");
    }

    const gatewayResult = await invokeGatewayTool("jira", "jira.create_issue", {
      title: issueTitle,
      description,
      project,
      issue_type: type,
    ...
    dataStore.addLog({
      ...
      service: "jira",
      action: "create_issue",
      message: `Created issue ${issueKey}: ${issueTitle}`,
  ...
  "jira.getIssues": async (params) => {
    ...
    const { jql, maxResults = 10 } = params as {
      jql: string;
      maxResults?: number;
    };
    ...
    const issues = Array.from(
      { length: Math.min(maxResults as number, 5) },
      (_, i) => ({
        key: `PROJ-${100 + i}`,
        fields: {
  ...
  return [
    {
      ...
      description: "Create a new Jira issue via MCP gateway",
    },
